...
#import "VAAdressPickerView.h"

@interface VAAdressPickerView ()<UIPickerViewDataSource, UIPickerViewDelegate>{
...
// 显示类型
@property (nonatomic, assign) VAAddressPickerMode showType;
// 选中的省
@property(nonatomic, strong) VAProvinceModel *selectProvinceModel;
// 选中的市
@property(nonatomic, strong) VACityModel *selectCityModel;
// 选中的区
@property(nonatomic, strong) VAAreaModel *selectAreaModel;
...
// 选中后的回调
@property (nonatomic, copy) VAAddressResultBlock resultBlock;
// 取消选择的回调
@property (nonatomic, copy) VAAddressCancelBlock cancelBlock;

@end

@implementation VAAdressPickerView

+ (void)showAddressPickerWithDefaultSelected:(NSArray *)defaultSelectedArr
                                 resultBlock:(VAAddressResultBlock)resultBlock {
    [self showAddressPickerWithShowType:VAAddressPickerModeArea dataSource:nil defaultSelected:defaultSelectedArr isAutoSelect:NO themeColor:nil resultBlock:resultBlock cancelBlock:nil];
}
...
+ (void)showAddressPickerWithDefaultSelected:(NSArray *)defaultSelectedArr
                                isAutoSelect:(BOOL)isAutoSelect
                                 ...
                                 resultBlock:(VAAddressResultBlock)resultBlock {
    [self showAddressPickerWithShowType:VAAddressPickerModeArea dataSource:nil defaultSelected:defaultSelectedArr isAutoSelect:isAutoSelect themeColor:themeColor resultBlock:resultBlock cancelBlock:nil];
}
...
+ (void)showAddressPickerWithShowType:(VAAddressPickerMode)showType
                      ...
                      defaultSelected:(NSArray *)defaultSelectedArr
                         isAutoSelect:(BOOL)isAutoSelect
                          ...
                          resultBlock:(VAAddressResultBlock)resultBlock
                          cancelBlock:(VAAddressCancelBlock)cancelBlock {
    VAAdressPickerView *addressPickerView = [[VAAdressPickerView alloc] initWithShowType:showType dataSource:dataSource defaultSelected:defaultSelectedArr isAutoSelect:isAutoSelect themeColor:themeColor resultBlock:resultBlock cancelBlock:cancelBlock];
...
- (instancetype)initWithShowType:(VAAddressPickerMode)showType
                      dataSource:(NSArray *)dataSource
                 defaultSelected:(NSArray *)defaultSelectedArr
                    isAutoSelect:(BOOL)isAutoSelect
                      themeColor:(UIColor *)themeColor
...
- (void)loadData {
    ...
    if (!self.dataSource || self.dataSource.count == 0) {
        /*
         ...
         然后再去找下面名为 VAPickerView 的 bundle 对象。
         */
        NSBundle *bundle = [NSBundle bundleForClass:[self class]];
        NSURL *url = [bundle URLForResource:@"VAPickerView" withExtension:@"bundle"];
        NSBundle *plistBundle = [NSBundle bundleWithURL:url];
        
        NSString *filePath = [plistBundle pathForResource:@"VACity" ofType:@"plist"];
...
- (void)parseDataSource {
    ...
    for (NSDictionary *proviceDic in self.dataSource) {
        VAProvinceModel *proviceModel = [[VAProvinceModel alloc]init];
        ...
        for (NSDictionary *cityDic in citylist) {
            VACityModel *cityModel = [[VACityModel alloc]init];
            ...
            for (NSDictionary *areaDic in arealist) {
                VAAreaModel *areaModel = [[VAAreaModel alloc]init];
...
- (void)setupDefaultValue {
    ...
    if (self.showType == VAAddressPickerModeCity || self.showType == VAAddressPickerModeArea) {
    ...
    if (self.showType == VAAddressPickerModeArea) {
...
- (void)scrollToRow:(NSInteger)provinceIndex secondRow:(NSInteger)cityIndex thirdRow:(NSInteger)areaIndex {
    if (self.showType == VAAddressPickerModeProvince) {
        [self.pickerView selectRow:provinceIndex inComponent:0 animated:YES];
    } else if (self.showType == VAAddressPickerModeCity) {
        [self.pickerView selectRow:provinceIndex inComponent:0 animated:YES];
        [self.pickerView selectRow:cityIndex inComponent:1 animated:YES];
    } else if (self.showType == VAAddressPickerModeArea) {
        [self.pickerView selectRow:provinceIndex inComponent:0 animated:YES];
        [self.pickerView selectRow:cityIndex inComponent:1 animated:YES];
...
- (NSArray *)getCityModelArray:(NSInteger)provinceIndex {
    VAProvinceModel *provinceModel = self.provinceModelArr[provinceIndex];
    // 返回城市模型数组
...
- (NSArray *)getAreaModelArray:(NSInteger)provinceIndex cityIndex:(NSInteger)cityIndex {
    VAProvinceModel *provinceModel = self.provinceModelArr[provinceIndex];
    VACityModel *cityModel = provinceModel.citylist[cityIndex];
    // 返回地区模型数组
...
- (NSInteger)numberOfComponentsInPickerView:(UIPickerView *)pickerView {
    switch (self.showType) {
        case VAAddressPickerModeProvince:
            return 1;
            break;
        case VAAddressPickerModeCity:
            return 2;
            break;
        case VAAddressPickerModeArea:
            return 3;
            break;
...
- (UIView *)pickerView:(UIPickerView *)pickerView viewForRow:(NSInteger)row forComponent:(NSInteger)component reusingView:(nullable UIView *)view {
    ...
    if (component == 0) {
        VAProvinceModel *model = self.provinceModelArr[row];
        label.text = model.name;
    }else if (component == 1){
        VACityModel *model = self.cityModelArr[row];
        label.text = model.name;
    }else if (component == 2){
        VAAreaModel *model = self.areaModelArr[row];
        label.text = model.name;
    }
    return bgView;
}
...
- (void)showWithAnimation:(BOOL)animation {
    ...
    if (animation) {
        ...
        [UIView animateWithDuration:0.3 animations:^{
            CGRect rect = self.alertView.frame;
            rect.origin.y -= kPickerHeight + kTopViewHeight + VA_BOTTOM_MARGIN;
            self.alertView.frame = rect;
        }];
    }
}
...
- (VAProvinceModel *)selectProvinceModel {
    if (!_selectProvinceModel) {
        _selectProvinceModel = [[VAProvinceModel alloc]init];
    }
    return _selectProvinceModel;
}

- (VACityModel *)selectCityModel {
    if (!_selectCityModel) {
        _selectCityModel = [[VACityModel alloc]init];
...
- (VAAreaModel *)selectAreaModel {
    if (!_selectAreaModel) {
        _selectAreaModel = [[VAAreaModel alloc]init];
